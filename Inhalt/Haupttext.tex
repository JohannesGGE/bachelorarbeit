...
Der erste Lösungsansatz, der beleuchtet wird ist die Verwendung von Keywords, welche druch den IMAP-Standard definert werden. Wie bereits erläutert können diese von den Clients erstellt und den einzelenen E-Mail zugeordnet werden. Es ist zwar möglich, dass der Mailserver das erstellen eigener Keywords nicht gestattet, was aber nur sehr selten der Fall ist.
...
\begin{itemize}
	\item \hl{Evt noch drauf eingehen, dass Wert von Eintrag mehrzeitlig sein kann und string NIL oder binary data sein kann}
	\item \hl{Evt auf min von 10 Einträge pro Mailbox müssen erlaubt sein, darüer hinaus vom server einschrenkbar, eingehen}
\end{itemize}
...
\hl{Es ist zu sehen}, dass es verschiedene Lösungsansätze gibt, die ermöglichen würden, eine Snooze-Funktion interoperabel zu implementieren. Dabei gibt es nicht \enquote{die} eine Lösung, sondern mehrere Teillösungen, die zusammen eine ganzheitliche Lösung schaffen könnten. 

\hl{Dabei haben die Teillösungen unterschiedliche Eigenschaften, die mehr oder weniger dazu beitragen, dass Sie von den Clients implementiert werden würden und die Grundlage für eine Standarderweiterung bereitstellen.} Die wohl wichtigste Eigenschaft, die einen positiven Beitrag leistet, ist die Verwendung des bereits existierenden Standards oder die Nutzung breit implementierter Erweiterungen des Standards. Hier können alle Ansätze Pluspunkte sammeln, lediglich der Teillösungen Metadata und Savedate können hier nicht auf vollstädnige Unterstützung setzen. Die Metadata Erweiterung ist dabei noch verbreiteter implementiert als die Savedate Erweterung.\\
...
Obwohl der Lösungsansatz mit Keywords als auch der Lösungsansatz, welcher die Header bearbeitet, für eine interoperable Lösung gleichermaßen geeignet sind, wird der Keyword Lösungsansatz für die Standarderweiterung \hl{vorgeschlagen/schlage ich vor?}. Grund dafür ist, dass das Bearbeiten der Header keine Aufgabe ist, die ein E-Mail-Client normalerweise macht und gegen das Konzept, was IMAP hat arbeitet. Des Weiteren müsste jeder Client diesen Mechanismus ersteinmal implementieren. Der Mechanismus zum setzen eines Keywords ist in den vielen E-Mail-Clients bereits vorhanden und bedarf nur eines geringen Implementierungsaufwandes.
...
\hl{Damit keine Speicher/Bandreibtenüberflutung stattfindet, soll der Mailserver regelmäßig Keyword , welche until abgelaufen ist löschen ODER einen Befehl zur verfügung stellen, mitdem sich das Keyword permanent entfernen lassen lässt.}

Im folgenden wird der Ablauf des Snoozens nocheinmal mit diesem Vorschlag in Form eines Sequenzdiagramms beschrieben....
