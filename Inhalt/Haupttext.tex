...
\hl{Wie bereits erwähnt} ist IMAP ein Protokoll um auf den mailstore eines Mailservers zuzugreifen. Es ist ein statusbehaftetes und bestätigendes Protokoll der Anwendungsschicht des OSI-Referenzmodells und nutzt das Transmission Control Protocol (TCP) zur übertragung. Die neuste Version von IMAP ist IMAP 4rev2 und definiert im RFC 9051\footnote{RFC9051 - Internet Message Access Protocol (IMAP) - Version 4rev2: \url{https://datatracker.ietf.org/doc/html/rfc9051}}.\\
...
Nextcloud als Kernsoftare bietet eine Vielzahl an Erweiterungen, welche durch sogenannte \textit{Apps} zur verfügung gestellt und einfach in der Software installiert und aktiviert werden können. Dazu zählen auch die Groupware Apps. \enquote{Nextcloud Groupware integriert Kalender, Kontakte, Mail und andere Produktivitätsfunktionen, und hilft Teams dadurch ihre Arbeit schneller, einfacher und zu ihren eigenen Spielregeln zu erledigen.} \cite{Groupware}
...
Wenn eine E-Mail in Gmail aus dem Posteingang \hl{gesnoozt} wird, wird das Label \enquote{Posteingang} entfernt und die E-Mail wird in Gmail unter dem Reiter \enquote{Zurückgestellt} bzw. \enquote{Snoozed} gelisted. Falls die E-Mail das Label \enquote{Posteingang} nicht mehr besitzt und ein anderes Label hat, wird dieses nicht entfernt. Durch das Entfernen des Label \enquote{Posteingang} wird die E-Mail über IMAP nicht mehr im Posteingang/Inbox angezeigt. Da Gmail den \hl{gesnoozten} E-Mails aber kein Label extra zuordnet, sind die E-Mails, welche aus dem Posteingang \hl{gesnoozt} werden, über IMAP nicht mehr auffindbar. Lediglich in einer Mailbox, welcher alle auf dem Gmail Account existierenden E-Mails beinhaltet, werden die \hl{gesnoozten} E-Mails noch angezeigt. \\
...
Die Snooze-Funktionalität ist ein kostenpflichtiges Feature in der Pro Version. \cite{Snooze} Allerdings lässt sich in der Android App auch ohne Pro \hl{snoozen}. Beim \hl{snoozen} wird der E-Mail ein Keyword zugeweisen, welches mit \enquote{\$SnoozedUntil\_} beginnt und von dem \hl{Unsnooze} Zeitpunkt als Unix Timestamp gefolgt wird. Daraus entsteht beispielsweise das folgende Keyword: \enquote{\$SnoozedUntil\_162597600}\\
...
\hl{Aufgaben, die erfüllt werden müssen sind, dass E-Mails, welche gesnoozt sind, für alle Clients in irgendeiner Weise als diese erkennbar sind.} Ebenfalls muss die Information, bis wann eine E-Mail gesnoozt ist, durch den Client ermittelbar sein. Auch muss sichergestellt werden, dass der aufweck Prozess einer E-Mail keine Inkosistez erzeugt, wenn Beispielsweise mehrere Clients gleichzeitig versuchen eine E-Mail aufzuwecken. \\
...
Ein Problem dieses Ansatztes ist, dass die Header einer E-Mail eig. nicht durch den empfangenden Client bearbeitet werden. \hl{Dazu gibts ja IMAP und Erweiterungen, um die Nachricht an sich unberührt zu lassen... Hier irgendwie noch begründen. Alle Clients müssten diese Funktionalität implementieren}
...
Hierführ bietet sich der RFC7162\footnote{RFC7162 - IMAP Extensions: Quick Flag Changes Resynchronization (CONDSTORE) and Quick Mailbox Resynchronization (QRESYNC): \url{https://datatracker.ietf.org/doc/html/rfc7162}} an. Diese Erweiterung ist Teil des Standards und weit verbreitet implementiert.\cite{boschSpecsUnofficialIMAP} Die \enquote{Conditional Store} Funktionalität ermöglicht es mehrere Änderungen innerhalb einer Mailbox zu koordinieren und sicherzustellen, dass immer nur ein Client schreibzugriff hat. Zusätzlich wird ein Mechanismus definiert, welcher es ermöglicht Mailbox Änderungen in Bezug auf Flags/Keywords und Metadaten schnell zu synconisieren.\cite{melnikovIMAPExtensionsQuick2014}
...
Die ermittelten Lösungsansäte oder Teillösungen werden jetzt auf ihre Tauglichkeit anhand der Nextcloud Mail App getestet. ...

Anstatt das Rad neu zu erfinden bedient sich die Nextcloud Mail App an der bereits existierenden IMAP Client library\footnote{IMAP Client library: \url{https://dev.horde.org/imap_client/index.php}} des Horde\footnote{Horde: \url{https://www.horde.org/}} Frameworks. \cite{MailAppsApp} Die IMAP Client library ist eine PHP library, welche eine vollständig abstrahierte Schnittstelle zu einem IMAP/POP3-Server bietet.\cite{HordeImap_ClientPHP} Laut der Webseite ist sie schneller, performanter und effizienter als die durch PHP mitgelieferte IMAP Extention\footnote{PHP IMAP Extention: \url{https://www.php.net/manual/de/book.imap.php}}.\cite{HordeImap_ClientPHPa} Die library ist streng RFC konform (IMAP4rev1) \cite{Imap_ClientLibraryHordea} und funktioniert mit allen gänigen IMAP Servern. \cite{HordeImap_ClientPHPa} Des Weiteren unterstützt sie die IMAP Extentions CONDSTORE und QRESYNC (RFC7162) sowie Access Control Lists (ACLs) (RFC2086, RFC4314). Darüber hinaus werden auch noch die weitere IMAP extentions unterstützt: MULTIAPPEND, BINARY, CATENATE, UIDPLUS, ESEARCH, THREAD/SORT, ESORT, LIST-STATUS, SPECIAL-USE, und MOVE.\cite{HordeImap_ClientPHPa}

Durch die RFC konfome Umsetzung des IMAP4rev1 Standards, werden Keywords unterstützt und der beschriebene Lösungsansatz mit Keywords ist möglich.\\
...
Für die schnelle und konflikfreie Resynchronisierung bietet die IMAP Client library eine Implementierung von den IMAP Extentions CONDSTORE und QRESYNC. Demnach ist auch diese Teillösung umsetzbar.
...
Mit dem gesammelten Erkentnissen kann nun ein Vorschlag für eine Erweiterung des IMAP-Standards erfolgen. Mithilfe dieses Vorschlages kann ein Internet Draft formuliert und im besten Fall ein Standardisierungprozess in gang gesetzt werden.


Es wird der Teillösungsansatz mit Keywords für die Standarderweiterung \hl{vorgeschlagen/schlage ich vor?}. Grund dafür ist die Nähe zum bereits existeirenden Standard und damit einhergehende serverseitige Unterstützung sowie die einfache Implementierung durch den Client, da die Grundlegenden Mechanismen bei den meisten Clients bereits Implementiert sind. 
...
Für eine schnelle und konflikfreie Synchonisation, wird die Implementierung des RFC7162 (IMAP Extensions: Quick Flag Changes Resynchronization (CONDSTORE) and Quick Mailbox Resynchronization (QRESYNC)) vorgeschlagen.

Abbildung X zeigt den Ablauf des Snoozens mit der vorgeschlagenen Lösung nocheinmal anschaulich.\\
...
Um einen Überblick zu bekommen, wurden verschiedene E-Mail-Clients auf eine bereits existierende Implementierung untersucht. Bei der Untersuchung hat sich heraugestellt, dass es bereits einige E-Mail-Clients mit einer Implementierung einer Snooze-Funktionalität gibt. Die Implementierungen waren aber in den meisten Fällen auf den Client oder den Dienst, den man benutzt, beschränkt. Eine interoperablen Ansatz haben nur der eM Client sowie das roundcube-snooze Plugin für den Roundcube Client aufgewiesen.\\
Als nächstes wurde disskutiert, welche Lösungsansätze es gibt und inwieweit diese für eine Standarderweiterung geeignet sind. Dazu wurde zuerst definiert, was eine Snooze-Funktionalität für Aufgaben erfüllen muss. Danach wurden sowohl die Lösungsansätze aus den existierenden Implementierungen, als auch neue Ideen auf eine theoretische Tauglichkeit untersucht. Dabei kam heraus, dass es nicht die eine perfekte Lösung gibt. Es gibt verschiedene Koponenten, die durch Ausnutzung existierender Funktionalität oder Nutzung, einen bestimmten Teil der Funktionalität umsetzen können. Aus einem Zusammenspiel mehrer dieser Komponenten lässt sich eine Gesamtlösung zusammenbauen.\\
Um eine Praxistauglichkeit zu bewerten, wurden die einzelnen Teillösungen anhand der Nextcloud Mail App untersucht. Dabei wurde festgestellt, dass ein es Teillösungen gab, die aus Sicht der Nextcould Mail App nicht für eine Implementierung geeignet sind, da der zusätzliche Aufwand extrem groß wäre.\\
