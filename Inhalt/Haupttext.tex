...
\begin{itemize}
	\item Etwas konkretisieren, sodass die Ziele abrechenbar sind.
\end{itemize}
...
\enquote{Ein Offener Standard ist zunächst ein technischer Standard, der in Freier und Offener Software eingesetzt wird. Es handelt sich dabei um eine Einigung auf eine gemeinsame Norm, die es ermöglicht, Informationen frei und ohne Veränderungen zu speichern (Format) oder zu übertragen (Protokoll). Das bedeutet, dass andere Menschen auf die Technik oder Inhalte Dritter aufbauen oder sie in anderen Kontexten weiterentwickeln können. Im Gegensatz dazu stünde ein \enquote{geschlossener} Standard, also z. B. eine proprietäre Anwendung, an der nur ein bestimmtes Unternehmen die Rechte hat, Veränderungen vorzunehmen oder überhaupt den Code einzusehen. Der Gedanke hinter Offenen Standards ist also der gleiche wie hinter Offener Software und Offenen Daten: Sie sollen öffentlich zugänglich, ihre Verwendung nicht rechtlich oder technisch eingeschränkt sein und die Beteiligung von Dritten ermöglichen.}\cite{OffeneStandardsUeberblick} \hl{Zu langes Zitat}
...
\hl{Wie bereits erwähnt} ist IMAP ein Protokoll, um auf den Mailstore eines Mailservers zuzugreifen. Es ist ein statusbehaftetes und bestätigendes Protokoll der Anwendungsschicht des OSI-Referenzmodells und nutzt das Transmission Control Protocol (TCP) zur Übertragung. Die neueste Version von IMAP ist IMAP 4rev2 und definiert im RFC 9051\footnote{RFC9051 - Internet Message Access Protocol (IMAP) - Version 4rev2: \url{https://datatracker.ietf.org/doc/html/rfc9051}}.\\
...
\textbf{\hl{Environment Implementierung}:} Client, welche die Snooze-Funktionalität besitzen und diese Clientübergreifend zumindest in der eigenen Umgebung funktionieren.\hl{Beispiel}\\
...
Bei der Analyse von bestehenden Implementierungen einer Snooze-Funktionalität haben sich einige E-Mail-Clients herausgestellt, welche diese Funktionalität gar nicht implementiert haben. Darunter fallen der Android-E-Mail-Client K-9 \footnote{K-9: \url{https://k9mail.app/}}, OpenExchangeMail \footnote{Open-Xchange Email: \url{https://www.open-xchange.com/products/ox-cloud}}, Yahoo Mail \footnote{Yahoo Mail: \url{https://overview.mail.yahoo.com/}}, Windows Live Mail\footnote{Windows Live Mail: \url{https://apps.microsoft.com/store/detail/mail-und-kalender/9WZDNCRFHVQM}}, SnappyMail \footnote{SnappyMail: \url{https://snappymail.eu/}}, Rainloop \footnote{Rainloop: \url{https://www.rainloop.net/}}, Tutanota \footnote{Tutanota: \url{https://tutanota.com/}}, Posteo \footnote{Posteo: \url{https://posteo.de/de}}, ProtonMail \footnote{ProtonMail: \url{https://proton.me/mail}}, Cypht \footnote{Cypht: \url{https://cypht.org/}} oder Evolution \footnote{Evolution: \url{https://wiki.gnome.org/Apps/Evolution}}.


\section{Lokale Implementierung}
\label{sec:LocalImpl}

\hl{Es gibt aber auch E-Mail-Clients}, welche die Snooze-Funktionalität implementiert haben, aber diese nur in dem jeweiligen Client nutzbar ist. Die folgenden Clients fallen darunter:
...
Clients, welche es auf verschiedenen Plattformen gibt, z.B. als App und Web-Client, haben meistens eine Syncronisation der Metadaten im Hintergrund über einen Nutzendenaccount. Dabei ist die Snooze-Funktionalität in diesen Clients meist übergreifend nutzbar, aber nicht über diese Umgebung hinaus. Die folgenden Clients bzw. Anbieter fallen darunter:
...
Roundcube\footnote{Roundcube: \url{https://roundcube.net/}} ist ein selbstgehosteter Webmailer und kann von sich aus nicht snoozen. Es gibt allerdings ein Plugin, roundcube-snooze\footnote{roundcube-snooze: \url{https://github.com/toptensoftware/roundcube-snooze}}, mit dem es möglich ist, E-Mails zu \hl{snoozen}. Beim \hl{Snoozen} wird dabei der Header der E-Mail bearbeitet. Beim Bearbeiten wird ein \hl{Header-Tag} \textit{X-Snoozed} hinzugefügt, welcher den Zeitpunkt des Snooze als auch den zukünftigen Zeitpunkt des \hl{Unsnooze} enthält. Außerdem wird der Name der Mailbox, aus der die E-Mail \hl{genoozt} wurde, und der aktuelle Status gespeichert.\cite{robinsonSnoozeRoundcube2023} Im Folgenden ist der Header-Tag einer Beispiel-E-Mail zu sehen, welche bereits \hl{Unsnoozt} ist.
...
Ein Problem dieses Ansatzes ist, dass die Header einer E-Mail eig. nicht durch den empfangenden Client bearbeitet werden. \hl{Dazu gibt es ja IMAP und Erweiterungen, um die Nachricht an sich unberührt zu lassen... Hier irgendwie noch begründen. Alle Clients müssten diese Funktionalität implementieren}
...
\begin{itemize}
	\item In welchem Standard Status befindet sich das???
\end{itemize}
...
Am wichtigsten ist die Verwendung des bereits existierenden Standards oder die Nutzung breits implementierter Erweiterungen des Standards. Sowohl die Verwendung von Keywords als auch die Special-Use-Mailbox setzen auf bereits standardisierte Mechanismen im IMAP-Protokoll. \hl{Bei der Verwendung von Keywords mit UNIX-Timestamp ist es zwar empfehlenswert, dass es einen Mechanismus gibt, der es ermöglicht, nicht mehr benutzte Keywords zu löschen, es funktioniert aber auch ohne}. Das Bearbeiten des Headers und das Erzeugen einer Referenz-E-Mail bedienen sich ebenfalls Mechanismen des IMAP-Protokolls. Zusätzlich wird dabei aber noch das E-Mail-Format an sich ausgenutzt, indem die E-Mail bearbeitet wird. Die Metadata-Erweiterung, die Savedate-Erweiterung und die Quick-Resynchonization sind Erweiterungen und damit optional für den Mailserver. Die serverseitige Implementierungen durch die Mailserver fallen dementsprechend unterschiedlich aus. Der \enquote{Conditional Store} Teil der Quick Resynchonization Erweiterung ist dabei noch am weitesten verbreitet implementiert. Die Metadata-Erweiterung und die Savedate-Erweiterung sind zwar in Dovecot implementiert, aber in anderen wichtigen Mailservern wie z.B. Gmail nicht.\\
...
Die ermittelten Lösungsansätze oder Teillösungen werden jetzt auf ihre Tauglichkeit anhand der Nextcloud Mail App getestet. ...
...
Beim Lösungsansatz mit der Metadaten-Erweiterung sieht es anders aus, die IMAP Client Library hat für diese Erweiterung keine Implementierung. Um eine Umsetzung in der Nextcloud Mail-App zu ermöglichen, müsste die IMAP Client library um diese Erweiterung angepasst werden. \hl{RAW IMAP COMMANDS??}\\
...
\hl{Kombi aus Keyword oder Header + Secial use + Constore/Qresync wäre am besten für Mail App}
