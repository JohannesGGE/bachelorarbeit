...
Der MTA realisiert den asynchronen Email Betrieb, indem er Emails vom Nutzendenclient versendet oder Emails von anderen MTAs empfängt und weiterleitet. Das Simple Mail Transport Protocol (SMTP) ist dabei das Protokoll, über dass die MTAs kommmunizieren.\\
Der MDA ist für die Bearbeitung der Email zuständig, wenn der aktuelle MTA das Ziel der Email ist. Er sogt dafür, dass die Email an der richtigen Stelle im mailstore (Email Speicher) für den adressierten Nutzenden abgespeichert wird.\\
Da SMTP sich zwar für den Versand von Emails, aber nicht für das nutzerseitige empfangen und abfragen eignet, gibt es noch den MUA. Der MUA ist die Schnittstelle zum Nutzenden und stellt Zugang zum mailstore durch ein Zugriffsprotokoll wie POP3 oder IMAP zur verfügung. Dabei ist der MUA sowohl Teil des Clients, den der Nutzende benutzt, als auch serverseitig um die Anfragen durch POP3 oder IMAP zu bearbeiten.\cite{mulletManagingIMAP2000} (Vgl. S. 4) \cite{heinleinDovecotPOP3IMAPServer2014} (Vgl. S. 20)\\
...
\begin{itemize}
	...
	\item was sind extenstions?
\end{itemize}

\begin{itemize}
	\item Protokoll der Sitzungsschicht
	\item Request-Response und Multiple-Request-Response Verfahrensweise
	\item zuverlässig, statefull, bestätigt
	\item sync von E-Mail zwischen Clients und Server ähnlich Dateisytemsynchnisation
	\item Nutzt Transmission Control Protocol (TCp)
\end{itemize}

\iffalse

IMAP is a way of accessing electronic mail that is stored on a central server. [...]
More precisely,IMAP is a way to retrieve messages from one or more mailboxes on a central server, without ever having to download a single message to local hard disk. The messages remain on the server at all times. [...]
With IMAP, you don?t have to wonder which computer you were on when you downloaded and read a given message. You know it?s still on the server. (S. 16)
...
MANAGING IMAP \cite{mulletManagingIMAP2000}
...
(Capaabilities) Drauf antwortet der Server mit einer Präsentaion seiner Fähigkeiten, den Capabilities. Nicht jeder Server muss alle IMAP-Features unterstützen: Die über das Kernprotokoll hinausgehenden IMAP-Erweiterungen, die IMAP-Extenstions, sind stets optional. (S. 45)

(List Command) Die Flags in der dritten Spalte verraten, ob das jeweilige Verzeichnis Unterordner (\textbackslash{HasChildren} - ja, \textbackslash{HasNoChildren} - nein) und Nachrichten beherbergt: In mit \textbackslash{Unmarked} gekennzeichnete Folder sind seit dem letzten Zugriff keine neuen Mails eingegenagen. (S. 46)

Der Server teilt uns mit dem Stichwort \texttt{FLAGS} die in diesem Ordner zulässigen Flags für E-Mails mit, denn diese sind ja nicht fest vorgegeben. Meist wird es sich dabei um folgende Handeln: \textbackslash{Answered}, Deleted, Draft, Flagged, Seen, Recen, \textbackslash{*} (S. 46-47)

Nun kann der Client Nachrichten ider auch nur Nachrichtenteile abrufen. Für den Aufbau des Inhaltsverzeichnisses reichen beispielsweise die Mail-header. Übder das Kommando \texttt{FETCH} lässt sich sehr genau steuern, welche E-Mails [...] und welche Teile dieser E-Mails übertragen werden. (S. 49)

(Keywords) Neben den oben vorgestellten fünf offiziellen IMAP-Flags haben IMAP-Clients die Möglichkeit, beliebige weitere, selbstdefinierte Flags zu benutzen. Diese nennt man auch Keywords; sie unterscheiden sich von den Systemflags lediglich durch den fehlenden vorangestellten Backslash im Namen: \textbackslash{Seen} ist eines der offiziellen IMAP-Flags, \texttt{Huhu} ein selbstdefiniertes.
Abgesehen von diesem Detail, werden selbstdefinierte und Systemflags auf gleiche Weise angesprochen und gesetzt.
Keyword können demnach wie due normalen IMAP-Flags temporärer Natur sein (dann gehen sie bei jedem Ordner-Wechsel verloren) oder permanenter auf dem Server gespeichert werden (dann bleiben sie auch nach dem Ausloggen erhalten). (S. 145-146)

(Extentions) IMAP ist nicht nur ein komplexes Protokoll, sondern auch ebenso komplex definiert. Neben RFC 3501, der IMAP in der seit März 2003 geltenden Version 4rev1 definiert, existieren unzählige weritre RFCs oder Internet-Drafts mit Erweiterungen, den IMAP-Extentions.
Diese Vielfalt verweundert nicht, hat das IMAP-Protokoll mit seinem Kommando \texttt{CAPABILITY} selbst den Grundstein dafür gelegt, dass Server flexibel optionale Erweiterungen anbieten können, die sich die Erfinder von IMAP nie hätte träumen lassen.
Die zahllosen Exentions sprengen leider den Rahmen dieserer Referenz, zumal dereren Unterstützung bei den IMAP-Clients häufig recht lückenhaft ist. (S. 348)
